...
window.ensureModalsExist = function() {
    ...
    if(!document.getElementById('ability-cutin')){
        const cutin = document.createElement('div'); cutin.id = 'ability-cutin'; cutin.className = 'hidden';
        cutin.innerHTML = `<div class="cutin-bg-red"></div><div class="cutin-bg-black"></div><div class="cutin-content"><img id="ability-cutin-img" src="" alt="Ability"><div class="cutin-text-container"><h2 id="ability-cutin-name"></h2><p id="ability-cutin-text"></p></div></div>`;
        document.body.appendChild(cutin);
    }
    if(!document.getElementById('ability-reset-overlay')){
        const resOver = document.createElement('div'); resOver.id = 'ability-reset-overlay'; resOver.className = 'hidden'; resOver.style.cssText = "position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(0,0,0,0.85); z-index:8000; display:flex; flex-direction:column; justify-content:center; align-items:center;";
        resOver.innerHTML = `<h2 style="color:white; margin-bottom:5px;">能力カード入れ替え (<span id="reset-timer" style="color:#fbc02d;">10</span>秒)</h2><p style="color:#ccc; font-size:12px; margin-top:0;">手札の能力カードをタップして入れ替えます(最大 <span id="reset-max">0</span> 枚)</p><div id="reset-area" style="width:90%; max-width:400px; height:100px; border:2px dashed #4caf50; border-radius:10px; display:flex; justify-content:center; align-items:center; gap:5px; background:rgba(255,255,255,0.1); overflow-x:auto;"></div><div id="reset-hand-area" style="margin-top:20px; display:flex; justify-content:center; flex-wrap:wrap; gap:5px; max-width:90%;"></div><button id="btn-reset-confirm" class="start-btn" style="width:200px; margin-top:20px;">確定</button>`;
        document.body.appendChild(resOver);
    }
    if(!document.getElementById('janken-overlay')){
        const jOverlay = document.createElement('div'); jOverlay.id = 'janken-overlay'; jOverlay.className = 'hidden'; jOverlay.style.cssText = "position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(0,0,0,0.95); z-index:110000; display:flex; flex-direction:column; justify-content:center; align-items:center; padding-bottom: 10vh; box-sizing: border-box;";
        jOverlay.innerHTML = `<h2 id="janken-title" style="color:white; font-style:italic; margin-bottom: 5px;">じゃんけん 勝負！</h2><div id="janken-subtitle" style="color:#aaa; font-size:14px; margin-bottom:15px;">Loop: 1/4</div><div style="color:#fbc02d; font-size:36px; font-weight:bold; margin-bottom:20px;">残り <span id="janken-timer">10</span> 秒</div><div style="display:flex; gap:30px; align-items:center; margin-bottom:30px; width: 90%; max-width: 500px; justify-content: center;"><div id="janken-player1" style="text-align:center; color:white; flex: 1;"><div id="janken-p1-result" style="font-size:24px; font-weight:bold; height:35px; text-shadow: 2px 2px 4px #000;"></div><div id="janken-p1-name" style="margin-bottom:10px; font-weight:bold; font-size:14px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">Player1</div><img id="janken-p1-card" src="${window.JANKEN_BACK_IMG}" style="width:80px; max-width: 100%; border-radius:10px; transition:0.3s; border-top: 10px solid transparent;"></div><div style="color:white; font-size:30px; font-weight:bold; font-style:italic;">VS</div><div id="janken-player2" style="text-align:center; color:white; flex: 1;"><div id="janken-p2-result" style="font-size:24px; font-weight:bold; height:35px; text-shadow: 2px 2px 4px #000;"></div><div id="janken-p2-name" style="margin-bottom:10px; font-weight:bold; font-size:14px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">Player2</div><img id="janken-p2-card" src="${window.JANKEN_BACK_IMG}" style="width:80px; max-width: 100%; border-radius:10px; transition:0.3s; border-top: 10px solid transparent;"></div></div><div id="janken-controls" style="display:flex; gap:15px;"><button class="janken-btn" data-hand="1" style="background:none; border:none; cursor:pointer; transition:transform 0.2s;"><img src="card/custom/jyanken1.png" style="width:70px;" onerror="this.outerHTML='<div style=\\'width:70px;height:70px;background:#fbc02d;color:black;border-radius:50%;display:flex;justify-content:center;align-items:center;font-weight:bold;font-size:18px;\\'>グー</div>'"></button><button class="janken-btn" data-hand="2" style="background:none; border:none; cursor:pointer; transition:transform 0.2s;"><img src="card/custom/jyanken2.png" style="width:70px;" onerror="this.outerHTML='<div style=\\'width:70px;height:70px;background:#fbc02d;color:black;border-radius:50%;display:flex;justify-content:center;align-items:center;font-weight:bold;font-size:18px;\\'>チョキ</div>'"></button><button class="janken-btn" data-hand="3" style="background:none; border:none; cursor:pointer; transition:transform 0.2s;"><img src="card/custom/jyanken3.png" style="width:70px;" onerror="this.outerHTML='<div style=\\'width:70px;height:70px;background:#fbc02d;color:black;border-radius:50%;display:flex;justify-content:center;align-items:center;font-weight:bold;font-size:18px;\\'>パー</div>'"></button></div>`;
            ...
            btn.onclick = () => { 
                ...
                if (window.isHost) {
                    if (window.pendingJanken && !window.pendingJanken.result) {
                        if (window.getMyId() === window.pendingJanken.attackerId) window.pendingJanken.attackerHand = choice;
                        if (window.getMyId() === window.pendingJanken.targetId) window.pendingJanken.targetHand = choice;
                        window.checkJankenReady();
                    }
                } else {
                    if (window.socket) window.socket.emit('player_action', { action: 'janken_choice', playerId: window.getMyId(), choice }); 
                }
                document.getElementById('janken-controls').style.display = 'none'; document.getElementById('janken-title').innerText = "相手を待っています..."; 
            }; 
        });
    }
};
...
const unlockAudioContext = () => {
    if (window.SE.unlocked) return; window.SE.unlocked = true; window.SE.initContext(); if (window.SE.audioCtx.state === 'suspended') window.SE.audioCtx.resume();
    ...
    ['fire', 'page'].forEach(name => window.SE.loadSound(name, 'wav')); document.removeEventListener('click', unlockAudioContext); document.removeEventListener('touchstart', unlockAudioContext);
};
document.addEventListener('click', unlockAudioContext); document.addEventListener('touchstart', unlockAudioContext, { passive: true });

window.initVolumeControl = function() {
    const controls = document.querySelectorAll('.volume-slider'); const icons = document.querySelectorAll('.volume-icon');
    controls.forEach(ctrl => { ctrl.addEventListener('input', (e) => { window.SE.setVolume(e.target.value); controls.forEach(c => c.value = e.target.value); icons.forEach(icon => icon.innerText = e.target.value == 0 ? '🔇' : '🔈'); }); });
};
...
window.handleDrawClick = function(e) {
    ...
    const optionalDraw = (window.RuleSettings && window.RuleSettings.optionalDraw === true);
    if (window.game.hasDrawnThisTurn && !optionalDraw) return;
    ...
    window.tryDrawWithAbility(() => {
        ...
        if (window.isHost) {
            ...
            setTimeout(() => { window.isDrawing = false; window.waitingForServerResponse = false; window.updateUI(); }, 2000);
        }
...
window.handleEndTurnClick = function(e) {
    ...
    if (window.isHost) {
        ...
        setTimeout(() => { window.waitingForServerResponse = false; window.isProcessingPlay = false; window.updateUI(); }, 2000);
    }
};
...
if (!window.hasBoundGlobalEvents) {
    ...
    document.body.addEventListener('click', (e) => {
        ...
        if (drawBtn) window.handleDrawClick(e);
        
        const endBtn = e.target.closest('#end-turn-btn');
        if (endBtn) window.handleEndTurnClick(e);
        
        const unoBtn = e.target.closest('#uno-btn');
        if (unoBtn) window.declareUno();
    });
}
...
window.updateUI = function() { 
    if(window.game && window.game.players && window.game.players.length > 0) { 
        ...
        if (drawBtns.length > 1) { for(let i = 1; i < drawBtns.length; i++) drawBtns[i].remove(); }
        const endBtns = document.querySelectorAll('#end-turn-btn');
        if (endBtns.length > 1) { for(let i = 1; i < endBtns.length; i++) endBtns[i].remove(); }
        ...
        if (drawBtn) {
            if (isMyTurnAndCanAct) {
                const optionalDraw = (window.RuleSettings && window.RuleSettings.optionalDraw === true);
                if (window.game.hasDrawnThisTurn && !optionalDraw) {
...
window.updatePhaseUI = function(state) {
    if (state.defensePhase) {
        ...
        if (targets && targets.includes(window.getMyId()) && window.getMyId() !== attackerId) {
            if (window.currentDefensePhaseId !== phaseId) { window.currentDefensePhaseId = phaseId; window.hasRespondedDefense = false; window.isDefending = true; if (typeof window.showDefenseModal === 'function') window.showDefenseModal(cardValue); }
            if (window.hasRespondedDefense) { const modal = document.getElementById('defense-modal'); const discModal = document.getElementById('discard-modal'); if (modal) modal.classList.add('hidden'); if (discModal) discModal.classList.add('hidden'); }
            const timerText = document.getElementById('defense-timer-text');
            if (timerText && state.defenseTimer !== undefined) timerText.innerText = state.defenseTimer;
        }
    } else {
    ...
    if (state.jankenPhase) {
        ...
        if (!state.jankenPhase.result) {
            ...
            if (p1Card && p1Card.src.indexOf('jyanken') > -1) { p1Card.src = window.JANKEN_BACK_IMG; p1Card.style.borderTop = '10px solid transparent'; }
            if (p2Card && p2Card.src.indexOf('jyanken') > -1) { p2Card.src = window.JANKEN_BACK_IMG; p2Card.style.borderTop = '10px solid transparent'; }
            ...
            if (iHaveChosen) {
                ...
                const controls = document.getElementById('janken-controls');
                if (controls) {
                    if (window.getMyId() === state.jankenPhase.attackerId || window.getMyId() === state.jankenPhase.targetId) { controls.style.display = 'flex'; }
                    else { controls.style.display = 'none'; if (title) title.innerText = "じゃんけん観戦中..."; }
                }
            }
        } else {
            if (!window.jankenResultPlayed) { window.jankenResultPlayed = true; if (typeof window.playJankenResult === 'function') window.playJankenResult(state.jankenPhase.attackerId, state.jankenPhase.targetId, state.jankenPhase.attackerHand, state.jankenPhase.targetHand, state.jankenPhase.result); }
        }
    } else {
        window.isJankenShowing = false; window.jankenResultPlayed = false; window.currentJankenLoopId = null; 
        const jOverlay = document.getElementById('janken-overlay'); if (jOverlay) { jOverlay.classList.remove('result-showing'); jOverlay.classList.add('hidden'); }
    }
};
...
window.broadcastGameState = function(skipUIUpdate = false, attackGuides = []) {
    ...
    if (attackGuides && attackGuides.length > 0) { attackGuides.forEach(g => { const delay = g.delay || 0; setTimeout(() => { if (typeof window.showAttackGuide === 'function') window.showAttackGuide(g.from, g.to, g.text, g.se); }, delay); }); }
    if (!window.hostSyncInterval) { window.hostSyncInterval = setInterval(() => { if (window.isHost && window.currentRoomState && window.currentRoomState.gameStarted) { window.broadcastGameState(true); } else { clearInterval(window.hostSyncInterval); window.hostSyncInterval = null; } }, 1500); }
};

window.playOpponentAnimation = function(playerId, cards, callback) {
    const badge = document.querySelector(`.other-player-badge[data-id="${playerId}"]`); const discardEl = document.getElementById('discard-pile');
    ...
    function animateSingle(index) {
        ...
        const endX = endRect.left + (endRect.width / 2) + (Math.floor(Math.random() * 11) - 5), endY = endRect.top + (endRect.height / 2) + (Math.floor(Math.random() * 11) - 5);
        if(typeof CardAnimation !== 'undefined') CardAnimation.moveCard(dummyCard, startRect, endX, endY, Math.floor(Math.random() * 21) - 10, () => {}); setTimeout(() => animateSingle(index + 1), 100);
    }
    animateSingle(0);
};
...
window.drawOpponentAnimation = function(playerId, count, callback) {
    if (window.SE) window.SE.playMultiple('Distribute', count, 500);
    const badge = document.querySelector(`.other-player-badge[data-id="${playerId}"]`); const deckEl = document.getElementById('deck-visual');
    ...
    function animateSingle(remaining) {
        ...
        if(typeof CardAnimation !== 'undefined') CardAnimation.moveCard(dummyCard, startRect, endX, endY, 0, () => {}); setTimeout(() => animateSingle(remaining - 1), 100);
    }
    animateSingle(count);
};
...
window.showAttackGuide = function(fromId, toId, labelText, seName) {
    ...
    let fromEl = document.querySelector(`.circle-player-badge[data-id="${fromId}"]`), toEl = document.querySelector(`.circle-player-badge[data-id="${toId}"]`);
    if (!fromEl && fromId === window.getMyId()) fromEl = document.getElementById('my-player-info'); if (!toEl && toId === window.getMyId()) toEl = document.getElementById('my-player-info');
    if (!fromEl || !toEl) return;
    const fromRect = fromEl.getBoundingClientRect(), toRect = toEl.getBoundingClientRect();
    const startX = fromRect.left + fromRect.width / 2, startY = fromRect.top + fromRect.height / 2; const endX = toRect.left + toRect.width / 2, endY = toRect.top + toRect.height / 2;
    const dx = endX - startX, dy = endY - startY; const dist = Math.sqrt(dx*dx + dy*dy); if (dist < 10) return;
    const midX = (startX + endX) / 2 - dy * 0.3, midY = (startY + endY) / 2 + dx * 0.3;
    const svgNS = "http://www.w3.org/2000/svg"; const svg = document.createElementNS(svgNS, "svg");
    svg.style.cssText = 'position:fixed; top:0; left:0; width:100vw; height:100vh; pointer-events:none; z-index:900000;'; 
    const defs = document.createElementNS(svgNS, "defs"), markerId = 'arrow-attack-' + fromId + toId + Date.now(), marker = document.createElementNS(svgNS, "marker");
    ...
    const markerPath = document.createElementNS(svgNS, "path"); markerPath.setAttribute('d', 'M 0 0 L 10 5 L 0 10 z'); markerPath.setAttribute('fill', '#ff5252'); marker.appendChild(markerPath); defs.appendChild(marker); svg.appendChild(defs);
    const path = document.createElementNS(svgNS, "path"); const d = `M ${startX},${startY} Q ${midX},${midY} ${endX},${endY}`;
    path.setAttribute('d', d); path.setAttribute('fill', 'none'); path.setAttribute('stroke', '#ff5252'); path.setAttribute('stroke-width', '3'); path.setAttribute('stroke-dasharray', '8 8'); path.setAttribute('marker-end', `url(#${markerId})`);
    const length = dist * 1.5; path.style.strokeDasharray = length; path.style.strokeDashoffset = length;
    const text = document.createElementNS(svgNS, "text"); text.setAttribute('x', midX); text.setAttribute('y', midY - 10); text.setAttribute('fill', '#fbc02d'); text.setAttribute('font-size', '16px'); text.setAttribute('font-weight', 'bold'); text.setAttribute('text-anchor', 'middle'); text.setAttribute('stroke', '#000'); text.setAttribute('stroke-width', '1'); text.textContent = labelText;
    svg.appendChild(path); svg.appendChild(text); document.body.appendChild(svg);
    path.animate([{ strokeDashoffset: length, opacity: 1 }, { strokeDashoffset: 0, opacity: 1, offset: 0.6 }, { strokeDashoffset: 0, opacity: 0 }], { duration: 1500, easing: 'ease-out', fill: 'forwards' });
    text.animate([{ opacity: 0, transform: `translateY(15px)` }, { opacity: 1, transform: `translateY(0px)`, offset: 0.2 }, { opacity: 1, transform: `translateY(0px)`, offset: 0.8 }, { opacity: 0, transform: `translateY(-15px)` }], { duration: 1500, easing: 'ease-out', fill: 'forwards' });
...
window.showAbilityResetUI = function(maxCount) {
    ...
    let selectedCards = [], myAbilities = window.game.myHand.filter(c => c && c.value && String(c.value).startsWith('id_'));
    ...
    const finish = () => { 
        ...
        if (selectedCards.length > 0) { 
            ...
            if (window.isHost) { window.game.replaceAbilityCards(window.getMyId(), vals); window.updateUI(); } 
            else { if(window.socket) window.socket.emit('player_action', { action: 'ability_reset', playerId: window.getMyId(), cards: vals }); } 
        } 
    };
...
window.animateInitialDeal = function(targetHands, callback) {
    ...
    const handSize = targetHands[playerIds[0]] ? targetHands[playerIds[0]].length : 0; const totalDeals = handSize * playerIds.length;
    ...
    const startRect = deckEl.getBoundingClientRect(); let dealIndex = 0; let completedDeals = 0; 
    function checkComplete() {
        completedDeals++;
        if (completedDeals >= totalDeals) {
            window.isDealAnimationRunning = false; window.game.hands = JSON.parse(JSON.stringify(targetHands)); window.updateUI(); window.isInitialDealing = false;
    ...
    function dealNext() {
        if (dealIndex >= totalDeals) return; const pIdx = dealIndex % playerIds.length; const round = Math.floor(dealIndex / playerIds.length); const pId = playerIds[pIdx];
        if (window.SE) window.SE.play('Distribute'); 
        const dummyCard = document.createElement('div'); dummyCard.className = 'card back'; dummyCard.style.width = '40px'; dummyCard.style.height = '60px';
        let endX, endY;
        if (pId === window.getMyId()) { const handEl = document.getElementById('player-hand'); const rect = handEl.getBoundingClientRect(); endX = rect.left + rect.width / 2; endY = rect.top + rect.height / 2; } else { const badge = document.querySelector(`.other-player-badge[data-id="${pId}"]`); if (badge) { const rect = badge.getBoundingClientRect(); endX = rect.left + rect.width / 2; endY = rect.top + rect.height / 2; } else { endX = window.innerWidth / 2; endY = window.innerHeight / 2; } }
        if (typeof CardAnimation !== 'undefined' && CardAnimation.moveCard) { CardAnimation.moveCard(dummyCard, startRect, endX, endY, 0, () => { if (targetHands[pId] && targetHands[pId][round]) { window.game.hands[pId].push(targetHands[pId][round]); window.updateUI(); } checkComplete(); }); } else { if (targetHands[pId] && targetHands[pId][round]) { window.game.hands[pId].push(targetHands[pId][round]); window.updateUI(); } checkComplete(); }
        dealIndex++; if (dealIndex < totalDeals) setTimeout(dealNext, 120); 
    }
    dealNext();
};

window.showAbilityCutin = function(cardValue, isHVActivated = false) {
    window.ensureModalsExist(); const cutinEl = document.getElementById('ability-cutin'), cutinImg = document.getElementById('ability-cutin-img'), cutinName = document.getElementById('ability-cutin-name'), cutinText = document.getElementById('ability-cutin-text');
    if (!cutinEl || !cutinImg) return; cutinImg.src = `card/custom/${cardValue}.png`;
    if (window.AbilityDef && window.AbilityDef[cardValue]) { if (cutinName) cutinName.innerText = window.AbilityDef[cardValue].name; if (cutinText) cutinText.innerText = window.AbilityDef[cardValue].desc; }
    cutinEl.classList.remove('hidden', 'fade-out'); void cutinEl.offsetWidth; cutinEl.classList.add('show-cutin');
    if (cardValue === 'id_20') { const rand = Math.random() < 0.5 ? 1 : 2; if (window.SE) window.SE.play(`hv/id_20(${rand})`); } else if (cardValue === 'id_25') { if (window.SE) window.SE.play(`hv/id_26`); } else if (cardValue === 'id_26') { if (window.SE) window.SE.play(`hv/id_25`); } else { const randMvp = Math.random() < 0.5 ? 1 : 2; if (window.SE) window.SE.play(`mvp_${randMvp}`); }
    setTimeout(() => { cutinEl.classList.remove('show-cutin'); cutinEl.classList.add('fade-out'); setTimeout(() => cutinEl.classList.add('hidden'), 400); }, 2000); 
};

window.openTargetSelection = function(players, callback) {
    window.ensureModalsExist(); const modal = document.getElementById('target-modal'), list = document.getElementById('target-modal-list');
    const validPlayers = players || (window.game && window.game.players) || []; const targets = validPlayers.filter(p => p.id !== window.getMyId());
    if (targets.length === 0) { callback(window.getMyId()); return; } if (targets.length === 1) { callback(targets[0].id); return; }
    list.innerHTML = ''; const defaultIcon = "data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='%23ccc'%3E%3Cpath d='M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z'/%3E%3C/svg%3E";
    targets.forEach(p => { const btn = document.createElement('div'); btn.className = 'target-icon-btn'; btn.innerHTML = `<img src="${p.icon || defaultIcon}"><span style="font-size:10px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; max-width:60px;">${p.name}</span>`; btn.onclick = () => { modal.classList.add('hidden'); callback(p.id); }; list.appendChild(btn); });
    modal.classList.remove('hidden');
};

window.openDiscardSelection = function(hand, excludeIndices, def, callback) {
    window.ensureModalsExist(); const modal = document.getElementById('discard-modal'), list = document.getElementById('discard-modal-list');
    const isAbilityDiscard = def && def.needsAbilityDiscard;
    if (!modal || !list) { const discardable = hand.findIndex((c, i) => { if (excludeIndices.includes(i)) return false; if (isAbilityDiscard) return (c && c.value && String(c.value).startsWith('id_')); return !(c && c.value && String(c.value).startsWith('id_')); }); callback(discardable > -1 ? discardable : 0); return; }
    list.innerHTML = ''; let addedCount = 0;
    hand.forEach((card, idx) => {
        if (!card || excludeIndices.includes(idx)) return; const isAb = card.value && String(card.value).startsWith('id_');
        if (isAbilityDiscard && !isAb) return; if (!isAbilityDiscard && def && def.needsDiscard && isAb) return;
        const btn = Renderer.createCardElement(card); btn.style.width = '50px'; btn.style.height = '75px'; btn.style.margin = '5px'; btn.style.cursor = 'pointer';
        btn.onclick = () => { modal.classList.add('hidden'); callback(idx); }; list.appendChild(btn); addedCount++;
    });
    if(addedCount === 0) { modal.classList.add('hidden'); callback(null); } else { modal.classList.remove('hidden'); }
};

window.openMultiDiscardSelection = function(hand, validIndices, callback) {
    window.ensureModalsExist(); const modal = document.getElementById('multi-discard-modal'), list = document.getElementById('multi-discard-modal-list'), btnConfirm = document.getElementById('btn-multi-discard-confirm');
    if (!modal || !list) { callback([]); return; }
    list.innerHTML = ''; let selectedIndices = [];
    validIndices.forEach(idx => {
        const card = hand[idx]; if(!card) return; const btn = Renderer.createCardElement(card); btn.style.width = '50px'; btn.style.height = '75px'; btn.style.margin = '5px'; btn.style.cursor = 'pointer'; btn.style.transition = '0.2s';
        btn.onclick = () => { const pos = selectedIndices.indexOf(idx); if (pos > -1) { selectedIndices.splice(pos, 1); btn.style.transform = 'translateY(0)'; btn.style.border = '1.5px solid white'; } else { selectedIndices.push(idx); btn.style.transform = 'translateY(-10px)'; btn.style.border = '2px solid #f1c40f'; } };
        list.appendChild(btn);
    });
    btnConfirm.onclick = () => { modal.classList.add('hidden'); callback(selectedIndices); }; modal.classList.remove('hidden');
};

window.openGraveyardSelection = function(callback) {
    window.ensureModalsExist(); const list = window.game.abilityGraveyard.filter(id => { const def = window.AbilityDef[id]; return def && def.rarity !== 'UR'; });
    if (list.length === 0) { window.showConfirm("対象となるSSR以下のカードが墓地にありません。\n適用されない効果がありますが、それでも実行しますか？", (yes) => { if(yes) callback(null); else callback(false); }); return; }
    const modal = document.getElementById('graveyard-modal'), container = document.getElementById('graveyard-list'); container.innerHTML = '';
    list.forEach(id => { const btn = document.createElement('img'); btn.src = `card/custom/${id}.png`; btn.style.width = '60px'; btn.style.cursor = 'pointer'; btn.onclick = () => { modal.classList.add('hidden'); callback(id); }; container.appendChild(btn); });
    document.getElementById('btn-cancel-graveyard').onclick = () => { modal.classList.add('hidden'); callback(false); }; modal.classList.remove('hidden');
};

window.openDebuffSelection = function(callback) {
    window.ensureModalsExist(); const modal = document.getElementById('debuff-modal'), btnFreeze = document.getElementById('btn-debuff-freeze'), btnBurn = document.getElementById('btn-debuff-burn');
    btnFreeze.onclick = () => { modal.classList.add('hidden'); callback('frozen'); }; btnBurn.onclick = () => { modal.classList.add('hidden'); callback('burn'); }; modal.classList.remove('hidden');
};
...
window.showDefenseModal = function(attackCardValue) {
    ...
    const sendResponse = (cardValue, discardIdx) => {
        ...
        if (window.isHost) {
            if (window.pendingDefense && window.pendingDefense.responses) { 
                window.pendingDefense.responses[window.getMyId()] = { cardValue: cardValue, discardIdx: discardIdx }; 
                if(window.pendingDefense.info && Object.keys(window.pendingDefense.responses).length >= window.pendingDefense.info.targets.length) window.pendingDefense.timer = 0; 
            }
        } else {
            if (window.socket) window.socket.emit('player_action', { action: 'defense_response', targetId: window.getMyId(), cardValue: cardValue, discardIdx: discardIdx });
        }
        window.isDefending = false;
    };

    if (!modal || !list) { sendResponse(null, null); return; }
    
    let attackName = '能力', attackDesc = ''; 
    if (window.AbilityDef && window.AbilityDef[attackCardValue]) { attackName = window.AbilityDef[attackCardValue].name; attackDesc = window.AbilityDef[attackCardValue].desc; } else if (attackCardValue === '+2' || attackCardValue === 'Wild+4') { attackName = attackCardValue; attackDesc = '引かされる枚数が増加します！(防御カードで防げます)'; }
    ...
    const myHand = window.game.myHand || []; const defCards = myHand.map((c, i) => ({card: c, idx: i})).filter(item => item.card && item.card.value && window.AbilityDef && window.AbilityDef[item.card.value] && window.AbilityDef[item.card.value].type.includes('BL'));
        ...
        defCards.forEach(item => {
            ...
            btn.onclick = () => {
                ...
                if (def && (def.needsDiscard || def.needsAbilityDiscard)) { 
                    ...
                    window.openDiscardSelection(myHand, [item.idx], def, (discIdx) => { sendResponse(item.card.value, discIdx); }); 
                } else { 
                    sendResponse(item.card.value, null); 
                }
            }; list.appendChild(btn);
        });
    };
    
    btnNo.onclick = () => { sendResponse(null, null); };
...
window.startDrawDefensePhase = function(attackerId, targetId, cardValue, guides) {
    try {
        ...
        if (p && p.type === 'bot') {
            const bHand = window.game.hands[targetId]; let hasDef = false;
            if(bHand) {
                const blIdx = bHand.findIndex(c => c && c.value && window.AbilityDef && window.AbilityDef[c.value] && window.AbilityDef[c.value].type.includes('BL'));
                if (blIdx > -1) {
                    const blCard = bHand[blIdx]; let bDiscard = null; const def = window.AbilityDef[blCard.value];
                    if(def && def.needsDiscard) { const nonAb = bHand.findIndex((c, i) => c && i !== blIdx && !(c.value && String(c.value).startsWith('id_'))); bDiscard = nonAb > -1 ? nonAb : (bHand.length > 1 ? bHand.findIndex((c, i) => c && i !== blIdx) : null); } else if (def && def.needsAbilityDiscard) { const ab = bHand.findIndex((c, i) => c && i !== blIdx && (c.value && String(c.value).startsWith('id_'))); bDiscard = ab > -1 ? ab : null; }
                    responses[targetId] = { cardValue: blCard.value, discardIdx: bDiscard }; hasDef = true;
                }
            }
            if(!hasDef) responses[targetId] = { cardValue: null, discardIdx: null };
        } else if (p && p.type === 'player' && !p.connected) { responses[targetId] = { cardValue: null, discardIdx: null }; }
        window.pendingDefense = { timer: 30, responses: responses, info: { attackerId: attackerId, cardValue: cardValue, targets: [targetId], phaseId: Date.now() } };
        if (Object.keys(responses).length >= 1) window.pendingDefense.timer = 0;
        window.broadcastGameState(false, guides);
        if (targetId === window.getMyId()) { window.isDefending = true; window.showDefenseModal(cardValue); }

        const interval = setInterval(() => {
            if (!window.pendingDefense) { clearInterval(interval); return; }
            if(window.pendingDefense.timer <= 0) {
                clearInterval(interval); const finalResponses = window.pendingDefense.responses; window.pendingDefense = null; let defenseGuides = []; let blocked = false;
                
                // 未回答のプレイヤーを強制的にnullで埋める（デッドロック防止）
                if (finalResponses && !finalResponses[targetId]) finalResponses[targetId] = { cardValue: null, discardIdx: null };

                if (finalResponses && finalResponses[targetId] && finalResponses[targetId].cardValue) {
                    const resp = finalResponses[targetId]; const defCardId = resp.cardValue; const tHand = window.game.hands[targetId]; const cIdx = tHand.findIndex(c => c && c.value === defCardId); let actualDefDiscardIdx = resp.discardIdx;
                    if (cIdx > -1) {
                        if (actualDefDiscardIdx !== null && cIdx < actualDefDiscardIdx) actualDefDiscardIdx--; const playedDefCard = tHand.splice(cIdx, 1)[0];
                        if(!window.game.abilityGraveyard) window.game.abilityGraveyard = []; window.game.abilityGraveyard.push(playedDefCard.value);
                        if (window.isHost && window.socket) window.socket.emit('request_play_animation', { playerId: targetId, cards: [playedDefCard] });
                        if (targetId !== window.getMyId() && typeof window.showAbilityCutin === 'function') { window.showAbilityCutin(defCardId); }
                        defenseGuides.push({ from: targetId, to: targetId, text: '防ぐ!', delay: 2500 }); blocked = true;
                    }
                    if (actualDefDiscardIdx !== null && tHand && tHand.length > actualDefDiscardIdx) {
                        ...
                        if (window.AbilityEngine && window.AbilityEngine.triggerDiscardEffect) { window.AbilityEngine.triggerDiscardEffect(window.game, targetId, discCard.value, true, discCard); }
                    ...
                    try {
                        ...
                        else if (defCardId === 'id_9') { window.game.players.filter(px => px.id !== targetId).forEach(px => { window.AbilityEngine.applyDraw(window.game, px.id, 2); }); }
                        else if (defCardId === 'id_18') { window.game.players.filter(px => px.id !== targetId).forEach(px => { window.AbilityEngine.applyDraw(window.game, px.id, 1); }); const targetP = window.game.players.find(p=>p.id===targetId); if(targetP) targetP.shield = { level: 1, turns: 2 }; }
                        else if (defCardId === 'id_19') { window.AbilityEngine.applyDraw(window.game, attackerId, 1); defenseGuides.push({ from: targetId, to: attackerId, text: 'ヴィンディ', delay: 2500 }); }
                        else if (defCardId === 'id_30') { const others = window.game.players.filter(px => px.id !== targetId && px.connected); if (others.length > 0) { const bt = others[Math.floor(Math.random() * others.length)]; window.AbilityEngine.applyBurn(window.game, bt.id, 1); defenseGuides.push({ from: targetId, to: bt.id, text: '🔥燃焼(1T)', delay: 2500 }); } const targetP = window.game.players.find(p=>p.id===targetId); if (targetP) targetP.shield = { level: 1, turns: 1 }; }
                        else if (defCardId === 'id_31') { const targetP = window.game.players.find(p=>p.id===targetId); if (targetP) { if (!targetP.shield) targetP.shield = { level: 0, turns: 0 }; targetP.shield.level += 3; targetP.shield.turns += 3; defenseGuides.push({ from: targetId, to: targetId, text: '🛡️シールド強化!', delay: 2500 }); } }
        ...
        console.error("Critical Error in Defense Phase:", err);
        if(window.isHost) { window.broadcastGameState(true); setTimeout(() => window.checkTurn(), 500); }
    }
};
...
window.executeAbilityPlay = function(playerId, indices, playedCardsData, targetId, discardIdx, selectedColor = null, multiDiscardIndices = [], extraData = {}) {
    try {
        if (!window.isHost) return;
        const hand = window.game.hands[playerId]; if (!hand || !playedCardsData || playedCardsData.length === 0) return; 
        
        multiDiscardIndices = multiDiscardIndices || [];
        extraData = extraData || {};
        
        const wasMyTurn = (window.game.currentPlayer && playerId === window.game.currentPlayer.id);
        ...
        if (extraData.isBlankPlay) {
            playedCardsData.forEach(pc => { 
                if(!pc) return;
                const idx = hand.findIndex(c => c && c.value === pc.value && c.color === pc.color); 
                if(idx > -1) { 
        ...
        if (multiDiscardIndices.length > 0) { safeMultiDiscardIndices = multiDiscardIndices.filter(i => { const c = originalHand[i]; return c && c.color === selectedColor && !(c.value && String(c.value).startsWith('id_')); }); }
        ...
        cardsToRemove.forEach(pc => { 
            ...
            if(idx > -1) { 
                const removed = hand.splice(idx, 1)[0]; 
                const isAb = removed && removed.value && String(removed.value).startsWith('id_'); 
                if (isAb) { if(!window.game.abilityGraveyard) window.game.abilityGraveyard = []; window.game.abilityGraveyard.push(removed.value); } 
                else if (removed) { window.game.discardPile.push(removed); window.game.discardRotations.push(0); } 
            } 
        });
        ...
        if (def.type === 'AT' || def.type === 'AT_BL' || def.type === 'HV') {
            ...
            else if (['id_13', 'id_14', 'id_24', 'id_28'].includes(cardValue)) { const others = window.game.players.filter(p=>p.id!==playerId); if(others.length > 0) targets = [others[Math.floor(Math.random()*others.length)].id]; } 
            else if (cardValue === 'id_5' || cardValue === 'id_20') { targets = [playerId]; }
            
            let responses = {}; let needsDefense = false;
            if (cardValue !== 'id_28') { 
                targets.forEach(tid => {
                    const p = window.game.players.find(px=>px.id===tid);
                    if(p && p.type==='bot') {
                        const bHand = window.game.hands[tid]; let hasDef = false;
                        if(bHand && def.type !== 'HV' && cardValue !== 'id_5' && cardValue !== 'id_20') {
                            const blIdx = bHand.findIndex(c => c && c.value && window.AbilityDef && window.AbilityDef[c.value] && window.AbilityDef[c.value].type.includes('BL'));
                            if (blIdx > -1) {
                                const blCard = bHand[blIdx]; let bDiscard = null; const bDef = window.AbilityDef[blCard.value];
                                if(bDef && bDef.needsDiscard) { const nonAb = bHand.findIndex((c, i) => c && i !== blIdx && !(c.value && String(c.value).startsWith('id_'))); bDiscard = nonAb > -1 ? nonAb : (bHand.length > 1 ? bHand.findIndex((c, i) => c && i !== blIdx) : null); } else if (bDef && bDef.needsAbilityDiscard) { const ab = bHand.findIndex((c, i) => c && i !== blIdx && (c.value && String(c.value).startsWith('id_'))); bDiscard = ab > -1 ? ab : null; }
                                responses[tid] = { cardValue: blCard.value, discardIdx: bDiscard }; hasDef = true;
                            }
                        }
                        if(!hasDef) responses[tid] = { cardValue: null, discardIdx: null };
                    } else if (p && p.type === 'player' && !p.connected) { responses[tid] = { cardValue: null, discardIdx: null }; } 
                    else if (def.type === 'HV' || cardValue === 'id_5' || cardValue === 'id_20') { responses[tid] = { cardValue: null, discardIdx: null }; } 
                    else { needsDefense = true; }
                });
            }
            ...
            if (cardValue === 'id_28' || !needsDefense) {
                ...
                try {
                    if (window.AbilityEngine && window.AbilityEngine.resolve) { 
                        ...
                        if (resGuides && Array.isArray(resGuides)) guides = guides.concat(resGuides);
                    }
                } catch(e) { console.error("Ability Resolve Error:", e); }
                guides.forEach(g => { if(g && g.delay === undefined) g.delay = 2500; }); 
            ...
            if (def.type !== 'HV' && cardValue !== 'id_5' && cardValue !== 'id_20' && targets.includes(window.getMyId()) && window.getMyId() !== playerId) { window.isDefending = true; window.showDefenseModal(cardValue); }
            ...
            const interval = setInterval(() => {
                ...
                if(window.pendingDefense.timer <= 0) {
                    ...
                    if (finalResponses) {
                        Object.keys(finalResponses).forEach(tid => {
                            ...
                            if (resp && resp.cardValue) {
                                const defCardId = resp.cardValue; const tHand = window.game.hands[tid]; const cIdx = tHand.findIndex(c => c && c.value === defCardId); let actualDefDiscardIdx = resp.discardIdx;
                                if (cIdx > -1) {
                                    if (actualDefDiscardIdx !== null && cIdx < actualDefDiscardIdx) actualDefDiscardIdx--; const playedDefCard = tHand.splice(cIdx, 1)[0];
                                    if(!window.game.abilityGraveyard) window.game.abilityGraveyard = []; window.game.abilityGraveyard.push(playedDefCard.value);
                                    if (window.isHost && window.socket) window.socket.emit('request_play_animation', { playerId: tid, cards: [playedDefCard] });
                                    if (tid !== window.getMyId() && typeof window.showAbilityCutin === 'function') { window.showAbilityCutin(defCardId); }
                                ...
                                try {
                                    ...
                                    else if (defCardId === 'id_19') { window.AbilityEngine.applyDraw(window.game, playerId, 1); guides.push({ from: tid, to: playerId, text: 'ヴィンディ', delay: 2500 }); }
                                    else if (defCardId === 'id_30') { const others = window.game.players.filter(px => px.id !== tid && px.connected); if (others.length > 0) { const bt = others[Math.floor(Math.random() * others.length)]; window.AbilityEngine.applyBurn(window.game, bt.id, 1); guides.push({ from: tid, to: bt.id, text: '🔥燃焼(1T)', delay: 2500 }); } const targetP = window.game.players.find(p=>p.id===tid); if (targetP) targetP.shield = { level: 1, turns: 1 }; }
                                    else if (defCardId === 'id_31') { const targetP = window.game.players.find(p=>p.id===tid); if (targetP) { if (!targetP.shield) targetP.shield = { level: 0, turns: 0 }; targetP.shield.level += 3; targetP.shield.turns += 3; guides.push({ from: tid, to: tid, text: '🛡️シールド強化!', delay: 2500 }); } }
                                } catch(e) { console.error("Defense Effect Error:", e); }
                            }
                        });
                    }
                    ...
                    try {
                        if (window.AbilityEngine && window.AbilityEngine.resolve) { 
                            const resGuides = window.AbilityEngine.resolve(window.game, playerId, cardValue, targetId, discCard, finalResponses, multiplier, selectedColor, multiCards, extraData); 
                            if (resGuides && Array.isArray(resGuides)) guides = guides.concat(resGuides); 
                        }
                    } catch(e) { console.error("Ability Resolve Error:", e); }
            ...
            try {
                if (window.AbilityEngine && window.AbilityEngine.resolve) { 
                    const resGuides = window.AbilityEngine.resolve(window.game, playerId, cardValue, targetId, discCard, {}, multiplier, selectedColor, multiCards, extraData); 
                    if (resGuides && Array.isArray(resGuides)) guides = guides.concat(resGuides); 
                }
            } catch(e) { console.error("Ability Resolve Error:", e); }
        ...
        console.error("Critical Error in executeAbilityPlay:", err);
        if (window.isHost) { window.broadcastGameState(true); setTimeout(() => window.checkTurn(), 500); }
    }
};
...
window.startJankenPhase = function(attackerId, loopCount, fixedTargetId = null, wasMyTurn = true) {
    ...
    window.jankenInterval = setInterval(() => {
        if (!window.pendingJanken || window.pendingJanken.result) { clearInterval(window.jankenInterval); return; }
        ...
        if (window.pendingJanken.timer <= 0) {
            ...
            [attackerId, targetId].forEach(id => { const p = window.game.players.find(px => px.id === id); if (p && p.type === 'bot' && !window.pendingJanken[(id===attackerId?'attackerHand':'targetHand')]) { if (window.pendingJanken.timer === 8) { window.pendingJanken[(id===attackerId?'attackerHand':'targetHand')] = Math.floor(Math.random()*3)+1; window.checkJankenReady(); } } });
...
window.resolveJanken = function() {
    if (window.jankenInterval) clearInterval(window.jankenInterval);
    try {
        const pJ = window.pendingJanken; const aH = pJ.attackerHand; const tH = pJ.targetHand; let result = 'draw';
        if (aH === tH) result = 'draw'; else if ((aH===1 && tH===2) || (aH===2 && tH===3) || (aH===3 && tH===1)) result = 'win'; else result = 'lose';
        pJ.result = result; window.broadcastGameState(); 
        setTimeout(() => {
            try {
                let drawCount = 0; if (result === 'win') { drawCount = 2; } else if (result === 'lose' && pJ.loopCount === 0) { drawCount = 2; }
                if (drawCount > 0) { 
                    if (window.AbilityEngine && window.AbilityEngine.applyDraw) window.AbilityEngine.applyDraw(window.game, pJ.targetId, drawCount, false); 
                ...
                console.error("Janken Inner Error:", err);
                window.pendingJanken = null; window.broadcastGameState(true); setTimeout(() => window.checkTurn(), 500);
            }
        }, 4500); 
    } catch(err) {
        console.error("Janken Resolve Error:", err);
        window.pendingJanken = null; window.broadcastGameState(true); setTimeout(() => window.checkTurn(), 500);
    }
};

window.showJankenUI = function(attackerId, targetId, loopCount) {
    window.ensureModalsExist(); let overlay = document.getElementById('janken-overlay'); if (!overlay) return;
    const jankenLoopId = `${attackerId}-${targetId}-${loopCount}`;
    if (window.currentJankenLoopId !== jankenLoopId) { window.currentJankenLoopId = jankenLoopId; if (loopCount > 0 && window.SE) window.SE.play('hv/id_26'); }
    overlay.classList.remove('result-showing', 'hidden');
    const title = document.getElementById('janken-title'), subtitle = document.getElementById('janken-subtitle'), p1Name = document.getElementById('janken-p1-name'), p2Name = document.getElementById('janken-p2-name'), p1Card = document.getElementById('janken-p1-card'), p2Card = document.getElementById('janken-p2-card'), p1Res = document.getElementById('janken-p1-result'), p2Res = document.getElementById('janken-p2-result'), controls = document.getElementById('janken-controls');
    if (title) title.innerText = `じゃんけん 勝負！`; if (subtitle) subtitle.innerText = `運命の三姉妹 (Loop: ${loopCount+1}/4)`;
    const aP = window.game.players.find(p=>p.id===attackerId), tP = window.game.players.find(p=>p.id===targetId);
    if (p1Name) p1Name.innerText = aP ? aP.name : 'Attacker'; if (p2Name) p2Name.innerText = tP ? tP.name : 'Target';
    if (p1Card) { p1Card.src = window.JANKEN_BACK_IMG; p1Card.style.borderTop = '10px solid transparent'; } if (p2Card) { p2Card.src = window.JANKEN_BACK_IMG; p2Card.style.borderTop = '10px solid transparent'; }
    if (p1Res) p1Res.innerText = ''; if (p2Res) p2Res.innerText = '';
    if (controls) { if (window.getMyId() === attackerId || window.getMyId() === targetId) { controls.style.display = 'flex'; } else { controls.style.display = 'none'; if (title) title.innerText = "じゃんけん観戦中..."; } }
};

window.playJankenResult = function(attackerId, targetId, aH, tH, result) {
    const overlay = document.getElementById('janken-overlay'); if (overlay) overlay.classList.add('result-showing'); 
    const p1Card = document.getElementById('janken-p1-card'), p2Card = document.getElementById('janken-p2-card'), p1Res = document.getElementById('janken-p1-result'), p2Res = document.getElementById('janken-p2-result'), controls = document.getElementById('janken-controls'), title = document.getElementById('janken-title');
    if (controls) controls.style.display = 'none'; if (title) title.innerText = "結果発表！";
    if (p1Card) { p1Card.src = `card/custom/jyanken${aH}.png`; p1Card.onerror = function() { this.src = "data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 40 60'%3E%3Crect width='40' height='60' rx='6' fill='%23555'/%3E%3Ctext x='50%25' y='50%25' dominant-baseline='middle' text-anchor='middle' fill='%23fff' font-size='16'%3E手" + aH + "%3C/text%3E%3C/svg%3E"; }; }
    if (p2Card) { p2Card.src = `card/custom/jyanken${tH}.png`; p2Card.onerror = function() { this.src = "data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 40 60'%3E%3Crect width='40' height='60' rx='6' fill='%23555'/%3E%3Ctext x='50%25' y='50%25' dominant-baseline='middle' text-anchor='middle' fill='%23fff' font-size='16'%3E手" + tH + "%3C/text%3E%3C/svg%3E"; }; }
    if (result === 'win') { if (p1Res) { p1Res.innerText = 'Win'; p1Res.style.color = '#fbc02d'; } if (p1Card) p1Card.style.borderTop = '10px solid #fbc02d'; if (p2Res) { p2Res.innerText = 'Lose'; p2Res.style.color = '#7e57c2'; } if (p2Card) p2Card.style.borderTop = '10px solid #7e57c2'; if (window.getMyId() === attackerId) { if(window.SE) window.SE.play('win'); } else if (window.getMyId() === targetId) { if(window.SE) window.SE.play('win2'); } } 
    else if (result === 'lose') { if (p1Res) { p1Res.innerText = 'Lose'; p1Res.style.color = '#7e57c2'; } if (p1Card) p1Card.style.borderTop = '10px solid #7e57c2'; if (p2Res) { p2Res.innerText = 'Win'; p2Res.style.color = '#fbc02d'; } if (p2Card) p2Card.style.borderTop = '10px solid #fbc02d'; if (window.getMyId() === targetId) { if(window.SE) window.SE.play('win'); } else if (window.getMyId() === attackerId) { if(window.SE) window.SE.play('win2'); } } 
    else { if (p1Res) { p1Res.innerText = 'Draw'; p1Res.style.color = '#aaa'; } if (p1Card) p1Card.style.borderTop = '10px solid #aaa'; if (p2Res) { p2Res.innerText = 'Draw'; p2Res.style.color = '#aaa'; } if (p2Card) p2Card.style.borderTop = '10px solid #aaa'; }
};

window.checkTurn = function() {
    try {
        if (!window.isHost || window.isGameOver || window.isInitialDealing) return; 
        clearInterval(window.turnTimer); const current = window.game.currentPlayer; if (!current) return;
        let dispName = current.name; if(current.type === 'bot' && window.RuleSettings && window.RuleSettings.showBotPersonality && current.personality) dispName += ` [${current.personality}]`;
        
        if (current.type === 'bot') {
            const roomStr = window.currentRoomState ? ` - 部屋ID:${window.currentRoomState.id}` : ""; document.getElementById('status-message').innerText = `${dispName} が考え中...${roomStr}`;
            window.broadcastGameState();
            setTimeout(() => {
                ...
                if (result.action === 'play') {
                    ...
                    if (isAbility && def) {
                        ...
                        if (def.needsAbilityDiscard) { const bHand = window.game.hands[current.id]; const discIdx = bHand.findIndex((c, i) => !result.indices.includes(i) && (c && c.value && String(c.value).startsWith('id_'))); botDiscardIdx = discIdx > -1 ? discIdx : null; } 
                        else if (def.needsDiscard) { const bHand = window.game.hands[current.id]; const discIdx = bHand.findIndex((c, i) => !result.indices.includes(i) && !(c && c.value && String(c.value).startsWith('id_'))); botDiscardIdx = discIdx > -1 ? discIdx : (bHand.length > result.indices.length ? bHand.findIndex((c,i)=>!result.indices.includes(i)) : null); }
                        if (playedCards[0].value === 'id_20' && botSelectedColor) { const bHand = window.game.hands[current.id]; bHand.forEach((c, i) => { if (!result.indices.includes(i) && c && c.color === botSelectedColor && !(c.value && String(c.value).startsWith('id_'))) botMultiDiscardIndices.push(i); }); willDiscard += botMultiDiscardIndices.length; }
                    }
                    ...
                    window.socket.emit('request_play_animation', { playerId: current.id, cards: playedCards }); const delay = playedCards.length * 100 + 400;
            ...
            const roomStr = window.currentRoomState ? ` - 部屋ID:${window.currentRoomState.id}` : ""; document.getElementById('status-message').innerText = current.id === window.getMyId() ? `あなたの番です${roomStr}` : `${dispName} のターン${roomStr}`;
            window.broadcastGameState();
            window.turnTimer = setInterval(() => {
                if (!window.playerAfkTimes[current.id]) window.playerAfkTimes[current.id] = 0; window.playerAfkTimes[current.id]++;
                if (window.playerAfkTimes[current.id] >= 180) { 
                    clearInterval(window.turnTimer); if(window.isGameOver || window.isInitialDealing) return; window.playerAfkTimes[current.id] = 0; 
                    setTimeout(() => {
                        const result = UNOBot.play(window.game, current.id);
                        if (result.action === 'play') {
                            const playedCards = result.indices.map(i => window.game.hands[current.id][i]); const isAbility = playedCards[0] && playedCards[0].value && String(playedCards[0].value).startsWith('id_'); const def = isAbility && window.AbilityDef ? window.AbilityDef[playedCards[0].value] : null;
    ...
    try {
        ...
        if (result.success) {
            ...
            if (result.needsColor) {
                ...
                if (isBot) { window.game.currentColor = ['red', 'blue', 'green', 'yellow'][Math.floor(Math.random() * 4)]; window.executeColor(playerId, window.game.currentColor); } 
                else if (playerId === window.getMyId()) { const roomStr = window.currentRoomState ? ` - 部屋ID:${window.currentRoomState.id}` : ""; document.getElementById('status-message').innerText = `色を選択してください${roomStr}`; ColorUI.show(); window.broadcastGameState(false, guides); } 
                else { const roomStr = window.currentRoomState ? ` - 部屋ID:${window.currentRoomState.id}` : ""; document.getElementById('status-message').innerText = `色選択中...${roomStr}`; window.socket.emit('request_color_select', playerId); window.broadcastGameState(false, guides); }
            } else { 
                if (isDrawAttack) {
                    targetId = window.game.currentPlayer.id; guides.push({ from: playerId, to: targetId, text: attackCardVal, delay: 0 }); 
                    if (window.RuleSettings && window.RuleSettings.customCards && window.RuleSettings.customCards.length > 0) { window.startDrawDefensePhase(playerId, targetId, attackCardVal, guides); } else { window.broadcastGameState(false, guides); window.checkTurn(); }
                } else { window.broadcastGameState(false, guides); window.checkTurn(); }
            }
        } else { if (isBot) window.executeDraw(playerId, true); else window.checkTurn(); }
    } catch(err) {
        console.error("ExecutePlay Error:", err);
        window.broadcastGameState(true); setTimeout(() => window.checkTurn(), 500);
    }
};
...
window.checkWin = function(playerId) {
    if (window.game.hands[playerId] && window.game.hands[playerId].length === 0) {
        window.isGameOver = true; if (window.SE) window.SE.stopLoop('final_sprint'); window.broadcastGameState(); 
        const winner = window.game.players.find(p => p.id === playerId); let winnerName = winner ? winner.name : "不明なプレイヤー";
        ...
        setTimeout(() => { if(window.socket) window.socket.emit('announce_win', { winnerId: playerId, winnerName: winnerName }); }, 500); return true;
    }
    return false;
};
...
window.animateSequentialPlay = function(indices, gameInstance, callback) {
    ...
    const playerHandEl = document.getElementById('player-hand'); if (!playerHandEl) { if(callback) callback(); return; }
    const currentIndex = indices[0]; const cardEl = playerHandEl.children[currentIndex];
    ...
    if (typeof CardAnimation !== 'undefined' && CardAnimation.animatePlay) { CardAnimation.animatePlay(cardEl, targetRotation, () => setTimeout(() => window.animateSequentialPlay(indices.slice(1), gameInstance, callback), 100)); } else window.animateSequentialPlay(indices.slice(1), gameInstance, callback);
};

window.declareUno = function() {
    if (window.isInitialDealing) return; window.game.unoDeclared = true; window.updateUI();
    const me = window.game.players.find(p => p.id === window.getMyId()); if(me && window.socket) window.socket.emit('declare_uno', { id: me.id, name: me.name });
};
...
window.handlePlayAction = function() {
    ...
    if (!window.game.isMyTurn && window.isServerProcessingAbility) {
        if(window.pendingDefense === null && window.pendingJanken === null) window.isServerProcessingAbility = false;
        else { alert("現在、他のプレイヤーの効果処理中です。解決するまでお待ちください。"); window.game.selectedIndices = []; window.updateUI(); return; }
    }
    ...
    if (!me) {
        console.error("自身のプレイヤー情報が見つかりません。");
        window.isProcessingPlay = false; 
    ...
    if (selectedCards.some(c => c && c.lockedTurns && c.lockedTurns > 0)) { alert("ロックされているカードは使用できません！"); window.game.selectedIndices = []; window.updateUI(); window.isProcessingPlay = false; return; }

    if (!isAbility) { const isLegalPlay = UNORules.canPlaySelected(selectedCards, window.game.topCard, window.game.currentColor, window.game.drawStack); if (!isLegalPlay) { if (window.SE) window.SE.play('Impossible'); window.isProcessingPlay = false; return; } }

    let willDiscard = (isAbility && def && (def.needsDiscard || def.needsAbilityDiscard)) ? 1 : 0; const finalHandCount = window.game.myHand.length - selectedCards.length - willDiscard;
    ...
    if (willBeActionFinishPenalty || willBeAbilityFinishPenalty || (finalHandCount <= 0 && !window.game.unoDeclared && window.RuleSettings && !window.RuleSettings.unoAuto)) {
        ...
        window.tryDrawWithAbility(() => {
            if (window.isHost) { 
                ...
                window.waitingForServerResponse = true; 
                if(window.socket) { window.socket.emit('player_action', { action: 'draw_penalty', playerId: currentId, count: penaltyCount, wasMyTurn: wasMyTurnLocal }); } 
            }
    ...
    if (isAbility) {
        let targetId = null, discardIdx = null, selColor = null, multiDiscardIndices = [], debuffToClear = null, graveyardCardId = null, extraData = {};
        ...
        const stepRaia = () => { if (cardValue === 'id_33' && window.game.isMyTurn && !me.usedRaia) { window.showConfirm("ライアの効果により、自分のターン終了時にこのカードを手札に戻しますか？\n(各ターン1回のみ)", (yes) => { if (yes) { extraData.returnRaia = true; me.usedRaia = true; } step6(); }); } else { step6(); } };
        const step5 = () => { if (def && def.needsDebuffSelect && me.frozen && me.burnTurns > 0) { window.openDebuffSelection((selected) => { debuffToClear = selected; stepRaia(); }); } else if (def && def.needsDebuffSelect) { if (me.frozen) debuffToClear = 'frozen'; else if (me.burnTurns > 0) debuffToClear = 'burn'; stepRaia(); } else { stepRaia(); } };
        const step4 = () => { if (def && cardValue === 'id_20' && selColor) { const validIndices = []; window.game.myHand.forEach((c, i) => { if (!indices.includes(i) && i !== discardIdx && c && c.color === selColor && !(c.value && String(c.value).startsWith('id_'))) validIndices.push(i); }); if (validIndices.length > 0) { window.openMultiDiscardSelection(window.game.myHand, validIndices, (selectedMulti) => { multiDiscardIndices = selectedMulti; step5(); }); } else { step5(); } } else { step5(); } };
        const step3 = () => { if (def && def.needsColor) { ColorUI.show((color) => { selColor = color; step4(); }); } else { step4(); } };
        const step2 = () => { if (def && (def.needsDiscard || def.needsAbilityDiscard)) { window.openDiscardSelection(window.game.myHand, indices, def, (idx) => { discardIdx = idx; step3(); }); } else { step3(); } };
        if (def && def.needsTarget) { window.openTargetSelection(window.game.players, (tid) => { targetId = tid; step2(); }); } else { step2(); }
        return; 
    }
...
(() => {
    ...
    let currentManualPage = 1; const totalManualPages = 8;
    function updateManualUI() { if(!manualImage) return; manualImage.src = `ex/ex${currentManualPage}.png`; if(manualPrev) manualPrev.classList.toggle('hidden-nav', currentManualPage === 1); if(manualNext) manualNext.classList.toggle('hidden-nav', currentManualPage === totalManualPages); if(manualClose) manualClose.classList.toggle('hidden', currentManualPage !== totalManualPages); let ind = ''; for (let i = 1; i <= totalManualPages; i++) ind += (i === currentManualPage) ? '🔸' : '🔹'; if(manualIndicators) manualIndicators.innerHTML = ind; }
    ...
    if(manualOverlay) {
        ...
        manualOverlay.addEventListener('touchend', (e) => { let endX = e.changedTouches[0].screenX; if (endX < touchStartX - 50 && currentManualPage < totalManualPages) { if(window.SE) window.SE.play('page'); currentManualPage++; updateManualUI(); } else if (endX > touchStartX + 50 && currentManualPage > 1) { if(window.SE) window.SE.play('page'); currentManualPage--; updateManualUI(); } }, { passive: true });
    }
})();
...
function initMainSocketEvents() {
    ...
    window.socket.on('sync_game_state', (state) => {
        ...
        else if (current) { const roomStr = window.currentRoomState ? ` - 部屋ID:${window.currentRoomState.id}` : ""; msgEl.innerText = current.id === window.getMyId() ? `あなたの番です${roomStr}` : `${current.name} のターン${roomStr}`; }
    ...
    window.socket.on('broadcast_uno', (data) => {
        if (window.SE) { const unoSounds = ['uno', 'uno2', 'uno3', 'uno4', 'uno5', 'uno6']; window.SE.play(unoSounds[Math.floor(Math.random() * unoSounds.length)]); }
        const target = (data.id === window.getMyId()) ? document.getElementById('my-player-info') : document.querySelector(`.circle-player-badge[data-id="${data.id}"]`);
    ...
    window.socket.on('game_over', (data) => {
        window.isGameOver = true; if (window.SE) window.SE.stopLoop('final_sprint');
        if (data.isDraw) {
            if (window.SE) window.SE.play('draw'); setTimeout(() => document.getElementById('draw-curtain').classList.add('show'), 100); setTimeout(() => { const b = document.getElementById('winner-banner'); b.innerText = "DRAW"; b.classList.add('show'); }, 3000);
        } else {
            if (window.SE) window.SE.play(Math.random() < 0.5 ? 'win' : 'win2'); const { winnerId, winnerName } = data; const target = (winnerId === window.getMyId()) ? document.getElementById('my-player-info') : document.querySelector(`.circle-player-badge[data-id="${winnerId}"]`); if(target) target.classList.add('winner-crown');
            setTimeout(() => { const b = document.getElementById('winner-banner'); b.innerText = `${winnerName} が勝ちました。`; b.classList.add('show'); if (typeof confetti === 'function') confetti({ particleCount: 150, spread: 80, origin: { x: 0.5, y: 0.5 } }); }, 5000);
        }
    ...
    window.socket.on('back_to_lobby', (roomState) => {
        ...
        const manualBtn = document.getElementById('btn-manual'); if (manualBtn) manualBtn.classList.remove('hidden');
        const cia = document.getElementById('chat-input-area'); if (cia) { cia.style.display = 'none'; cia.classList.add('hidden'); }
        const lcia = document.getElementById('lobby-chat-container'); if (lcia && window.ChatManager && window.ChatManager.enabled) lcia.style.display = 'block';
    ...
    window.socket.on('receive_player_action', (data) => {
        ...
        if (data.action === 'play') {
            ...
            if (window.pendingDefense || window.pendingJanken) return;
            if (!data.cards || !data.indices) return;
    ...
    window.socket.on('play_animation', (data) => {
        ...
        if (window.animatingTimeout) clearTimeout(window.animatingTimeout);
        window.animatingTimeout = setTimeout(() => { window.isAnimating = false; }, (data.cards.length * 100) + 2000);
        if (data.playerId !== window.getMyId()) { if (typeof window.playOpponentAnimation === 'function') window.playOpponentAnimation(data.playerId, data.cards); if (data.cards && data.cards.length > 0 && data.cards[0].value && String(data.cards[0].value).startsWith('id_')) { if (typeof window.showAbilityCutin === 'function') window.showAbilityCutin(data.cards[0].value, data.isHV); } }
    });

    window.socket.on('draw_animation', (data) => {
        window.isAnimating = true;
        if (window.animatingTimeout) clearTimeout(window.animatingTimeout);
        window.animatingTimeout = setTimeout(() => { window.isAnimating = false; }, (data.count * 100) + 1000);
        if (data.playerId !== window.getMyId()) { if (typeof window.drawOpponentAnimation === 'function') window.drawOpponentAnimation(data.playerId, data.count); }
    });
