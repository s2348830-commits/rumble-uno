            ...
            btn.onclick = () => { 
                ...
                if (window.isHost) {
                    if (window.pendingJanken && !window.pendingJanken.result) {
                        if (window.myId === window.pendingJanken.attackerId) window.pendingJanken.attackerHand = choice;
                        if (window.myId === window.pendingJanken.targetId) window.pendingJanken.targetHand = choice;
...
window.updatePhaseUI = function(state) {
    ...
    if (state.jankenPhase) {
        ...
        if (!state.jankenPhase.result) {
            ...
            const iHaveChosen = (window.myId === state.jankenPhase.attackerId && state.jankenPhase.attackerHand) || (window.myId === state.jankenPhase.targetId && state.jankenPhase.targetHand);
            ...
            if (iHaveChosen) {
                ...
                const controls = document.getElementById('janken-controls');
                if (controls) {
                    if (window.myId === state.jankenPhase.attackerId || window.myId === state.jankenPhase.targetId) { controls.style.display = 'flex'; }
...
window.showAttackGuide = function(fromId, toId, labelText, seName) {
    ...
    if (!fromEl && fromId === window.game.myId) fromEl = document.getElementById('my-player-info'); if (!toEl && toId === window.game.myId) toEl = document.getElementById('my-player-info');
...
window.showAbilityResetUI = function(maxCount) {
    ...
    if(!overlay) return; let timeLeft = 10; maxSpan.innerText = maxCount; resetArea.innerHTML = ''; handArea.innerHTML = '';
    ...
    const finish = () => { clearInterval(timerInt); overlay.classList.add('hidden'); if (selectedCards.length > 0) { const vals = selectedCards.map(c => c.value); if (window.isHost) { window.game.replaceAbilityCards(window.game.myId, vals); window.updateUI(); } else { if(window.socket) window.socket.emit('player_action', { action: 'ability_reset', cards: vals }); } } };
...
window.openTargetSelection = function(players, callback) {
    ...
    const validPlayers = players || (window.game && window.game.players) || []; const targets = validPlayers.filter(p => p.id !== window.game.myId);
    if (targets.length === 0) { callback(window.game.myId); return; } if (targets.length === 1) { callback(targets[0].id); return; }
...
window.openDiscardSelection = function(hand, excludeIndices, def, callback) {
    ...
    if (!modal || !list) { const discardable = hand.findIndex((c, i) => { if (excludeIndices.includes(i)) return false; if (isAbilityDiscard) return (c.value && String(c.value).startsWith('id_')); return !(c.value && String(c.value).startsWith('id_')); }); callback(discardable > -1 ? discardable : 0); return; }
    list.innerHTML = ''; let addedCount = 0;
    hand.forEach((card, idx) => {
        if (excludeIndices.includes(idx)) return; const isAb = card.value && String(card.value).startsWith('id_');
...
window.openMultiDiscardSelection = function(hand, validIndices, callback) {
    ...
    validIndices.forEach(idx => {
        const card = hand[idx]; const btn = Renderer.createCardElement(card); btn.style.width = '50px'; btn.style.height = '75px'; btn.style.margin = '5px'; btn.style.cursor = 'pointer'; btn.style.transition = '0.2s';
    ...
    const sendResponse = (cardValue, discardIdx) => {
        ...
        if (window.isHost) {
            if (window.pendingDefense && window.pendingDefense.responses) { 
                window.pendingDefense.responses[window.game.myId] = { cardValue: cardValue, discardIdx: discardIdx }; 
            ...
            if (window.socket) window.socket.emit('player_action', { action: 'defense_response', targetId: window.game.myId, cardValue: cardValue, discardIdx: discardIdx });
        }
...
window.startDrawDefensePhase = function(attackerId, targetId, cardValue, guides) {
    ...
    if (p && p.type === 'bot') {
        ...
        if(bHand) {
            ...
            if (blIdx > -1) {
                ...
                if(def && def.needsDiscard) { const nonAb = bHand.findIndex((c, i) => i !== blIdx && !(c.value && String(c.value).startsWith('id_'))); bDiscard = nonAb > -1 ? nonAb : (bHand.length > 1 ? bHand.findIndex((c, i) => i !== blIdx) : null); } else if (def && def.needsAbilityDiscard) { const ab = bHand.findIndex((c, i) => i !== blIdx && (c.value && String(c.value).startsWith('id_'))); bDiscard = ab > -1 ? ab : null; }
    ...
    if (targetId === window.game.myId) { window.isDefending = true; window.showDefenseModal(cardValue); }
    ...
    const interval = setInterval(() => {
        ...
        if(window.pendingDefense.timer <= 0) {
            ...
            if (finalResponses && finalResponses[targetId] && finalResponses[targetId].cardValue) {
                const resp = finalResponses[targetId]; const defCardId = resp.cardValue; const tHand = window.game.hands[targetId]; const cIdx = tHand.findIndex(c => c.value === defCardId); let actualDefDiscardIdx = resp.discardIdx;
                if (cIdx > -1) {
                    ...
                    if (targetId !== window.game.myId && typeof window.showAbilityCutin === 'function') { window.showAbilityCutin(defCardId); }
...
window.executeAbilityPlay = function(playerId, indices, playedCardsData, targetId, discardIdx, selectedColor = null, multiDiscardIndices = [], extraData = {}) {
    ...
    if (def.type === 'AT' || def.type === 'AT_BL' || def.type === 'HV') {
                ...
                if(p && p.type==='bot') {
                    ...
                    if(bHand && def.type !== 'HV' && cardValue !== 'id_5' && cardValue !== 'id_20') {
                        ...
                        if (blIdx > -1) {
                            ...
                            if(bDef && bDef.needsDiscard) { const nonAb = bHand.findIndex((c, i) => i !== blIdx && !(c.value && String(c.value).startsWith('id_'))); bDiscard = nonAb > -1 ? nonAb : (bHand.length > 1 ? bHand.findIndex((c, i) => i !== blIdx) : null); } else if (bDef && bDef.needsAbilityDiscard) { const ab = bHand.findIndex((c, i) => i !== blIdx && (c.value && String(c.value).startsWith('id_'))); bDiscard = ab > -1 ? ab : null; }
        ...
        if (def.type !== 'HV' && cardValue !== 'id_5' && cardValue !== 'id_20' && targets.includes(window.game.myId) && window.game.myId !== playerId) { window.isDefending = true; window.showDefenseModal(cardValue); }
                ...
                if (finalResponses) {
                    Object.keys(finalResponses).forEach(tid => {
                        ...
                        if (resp && resp.cardValue) {
                            const defCardId = resp.cardValue; const tHand = window.game.hands[tid]; const cIdx = tHand.findIndex(c => c.value === defCardId); let actualDefDiscardIdx = resp.discardIdx;
                            if (cIdx > -1) {
                                if (actualDefDiscardIdx !== null && cIdx < actualDefDiscardIdx) actualDefDiscardIdx--; const playedDefCard = tHand.splice(cIdx, 1)[0];
                                if(!window.game.abilityGraveyard) window.game.abilityGraveyard = []; window.game.abilityGraveyard.push(playedDefCard.value);
                                if (window.isHost && window.socket) window.socket.emit('request_play_animation', { playerId: tid, cards: [playedDefCard] });
                                if (tid !== window.game.myId && typeof window.showAbilityCutin === 'function') { window.showAbilityCutin(defCardId); }
...
window.showJankenUI = function(attackerId, targetId, loopCount) {
    ...
    if (p1Res) p1Res.innerText = ''; if (p2Res) p2Res.innerText = '';
    if (controls) { if (window.myId === attackerId || window.myId === targetId) { controls.style.display = 'flex'; } else { controls.style.display = 'none'; if (title) title.innerText = "じゃんけん観戦中..."; } }
};
...
window.playJankenResult = function(attackerId, targetId, aH, tH, result) {
    ...
    if (result === 'win') { if (p1Res) { p1Res.innerText = 'Win'; p1Res.style.color = '#fbc02d'; } if (p1Card) p1Card.style.borderTop = '10px solid #fbc02d'; if (p2Res) { p2Res.innerText = 'Lose'; p2Res.style.color = '#7e57c2'; } if (p2Card) p2Card.style.borderTop = '10px solid #7e57c2'; if (window.myId === attackerId) { if(window.SE) window.SE.play('win'); } else if (window.myId === targetId) { if(window.SE) window.SE.play('win2'); } } 
    else if (result === 'lose') { if (p1Res) { p1Res.innerText = 'Lose'; p1Res.style.color = '#7e57c2'; } if (p1Card) p1Card.style.borderTop = '10px solid #7e57c2'; if (p2Res) { p2Res.innerText = 'Win'; p2Res.style.color = '#fbc02d'; } if (p2Card) p2Card.style.borderTop = '10px solid #fbc02d'; if (window.myId === targetId) { if(window.SE) window.SE.play('win'); } else if (window.myId === attackerId) { if(window.SE) window.SE.play('win2'); } } 
...
window.checkTurn = function() {
    ...
    if (current.type === 'bot') {
        ...
        setTimeout(() => {
            ...
            if (result.action === 'play') {
                ...
                if (isAbility && def) {
                    ...
                    if (def.needsAbilityDiscard) { const bHand = window.game.hands[current.id]; const discIdx = bHand.findIndex((c, i) => !result.indices.includes(i) && (c.value && String(c.value).startsWith('id_'))); botDiscardIdx = discIdx > -1 ? discIdx : null; } 
                    else if (def.needsDiscard) { const bHand = window.game.hands[current.id]; const discIdx = bHand.findIndex((c, i) => !result.indices.includes(i) && !(c.value && String(c.value).startsWith('id_'))); botDiscardIdx = discIdx > -1 ? discIdx : (bHand.length > result.indices.length ? bHand.findIndex((c,i)=>!result.indices.includes(i)) : null); }
                    if (playedCards[0].value === 'id_20' && botSelectedColor) { const bHand = window.game.hands[current.id]; bHand.forEach((c, i) => { if (!result.indices.includes(i) && c.color === botSelectedColor && !(c.value && String(c.value).startsWith('id_'))) botMultiDiscardIndices.push(i); }); willDiscard += botMultiDiscardIndices.length; }
                }
        ...
        const roomStr = window.currentRoomState ? ` - 部屋ID:${window.currentRoomState.id}` : ""; document.getElementById('status-message').innerText = current.id === window.game.myId ? `あなたの番です${roomStr}` : `${dispName} のターン${roomStr}`;
        window.broadcastGameState();
...
window.executePlay = function(playerId, indices, isBot = false) {
    ...
    if (result.success) {
        ...
        if (result.needsColor) {
            ...
            else if (playerId === window.game.myId) { const roomStr = window.currentRoomState ? ` - 部屋ID:${window.currentRoomState.id}` : ""; document.getElementById('status-message').innerText = `色を選択してください${roomStr}`; ColorUI.show(); window.broadcastGameState(false, guides); } 
...
window.declareUno = function() {
    ...
    const me = window.game.players.find(p => p.id === window.game.myId); if(me && window.socket) window.socket.emit('declare_uno', { id: me.id, name: me.name });
};

window.tryDrawWithAbility = function(callback) {
    const me = window.game.players.find(p => p.id === window.game.myId); const renaIdx = window.game.myHand.findIndex(c => c.value === 'id_9');
...
window.handlePlayAction = function() {
    ...
    // 【改善案1】クリックした瞬間に、古いロックを強制掃除する。
    // アクションポップアップ（対象選択など）が出ていなければ、処理中フラグを落とす
    if (!document.querySelector('.action-popup:not(.hidden)')) {
        window.isProcessingPlay = false;
    }
    
    // 各種ロック判定
    if (window.waitingForServerResponse) {
        alert("サーバーと通信中です。少しお待ちください。"); window.game.selectedIndices = []; window.updateUI(); return;
    }
    if (window.pendingJanken || window.pendingDefense || window.currentDefensePhaseId || window.isDefending) {
        alert("現在、効果処理または防御処理中です。解決するまでお待ちください。"); window.game.selectedIndices = []; window.updateUI(); return;
    }
    if (window.isProcessingPlay) {
        alert("現在、カードの処理中です。"); window.game.selectedIndices = []; window.updateUI(); return;
    }
    
    // 【改善案2】参加者側のブロック機能の二重チェック
    if (!window.game.isMyTurn && window.isServerProcessingAbility) {
        // もしサーバー側で何も起きていないならロックを自動解除
        if(window.pendingDefense === null && window.pendingJanken === null) {
            window.isServerProcessingAbility = false;
        } else {
            alert("現在、他のプレイヤーの効果処理中です。解決するまでお待ちください。");
            ...
            if (yes) {
                ...
                window.animateSequentialPlay(indicesToSend, window.game, () => {
                    if (window.isHost) { 
                        ...
                        window.executeAbilityPlay(window.game.myId, indicesToSend, cardsToSend, null, null, null, [], { isBlankPlay: true }); 
                    } else { 
                        window.waitingForServerResponse = true; 
                        if (window.socket) window.socket.emit('player_action', { action: 'play_ability', indices: indicesToSend, cards: cardsToSend, targetId: null, discardIdx: null, selectedColor: null, multiDiscardIndices: [], isHV: false, extraData: { isBlankPlay: true } }); 
                    }
                    window.isProcessingPlay = false; 
                });
            } else {
    ...
    if (willBeActionFinishPenalty || willBeAbilityFinishPenalty || (finalHandCount <= 0 && !window.game.unoDeclared && window.RuleSettings && !window.RuleSettings.unoAuto)) {
        ...
        window.game.selectedIndices = []; window.updateUI(); alert(`${penaltyMsg} ペナルティとして ${penaltyCount}枚ドローします！`); 
        ...
        window.tryDrawWithAbility(() => {
            ...
            const finishDraw = () => {
                ...
                if (window.isHost) { 
                    ...
                    window.waitingForServerResponse = true; if(window.socket) window.socket.emit('player_action', { action: 'draw_penalty', count: penaltyCount, wasMyTurn: wasMyTurnLocal }); 
                }
            };
            if (typeof CardAnimation !== 'undefined' && CardAnimation.animateMultiDraw) {
                CardAnimation.animateMultiDraw(penaltyCount, 'player-hand', finishDraw);
            } else {
                finishDraw();
            }
        }); 
    ...
    if (isAbility) {
        ...
        const finishAbilityPlay = () => {
            window.game.selectedIndices = []; window.updateUI();
            const indicesToSend = [...indices], cardsToSend = [...playedCards];
            ...
            window.animateSequentialPlay(indices, window.game, () => {
                ...
                if (window.isHost) { 
                    ...
                    window.executeAbilityPlay(window.game.myId, indicesToSend, cardsToSend, targetId, discardIdx, selColor, multiDiscardIndices, extraData); 
                } else { 
                    window.waitingForServerResponse = true; 
                    if (window.socket) window.socket.emit('player_action', { action: 'play_ability', indices: indicesToSend, cards: cardsToSend, targetId: targetId, discardIdx: discardIdx, selectedColor: selColor, multiDiscardIndices, isHV: isHVActivated, extraData }); 
                }
                window.isProcessingPlay = false; 
            });
        };

        const step6 = () => { if (def && def.needsGraveyard) { window.openGraveyardSelection((id) => { if (id === false) { window.game.selectedIndices = []; window.updateUI(); window.isProcessingPlay = false; return; } graveyardCardId = id; finishAbilityPlay(); }); } else { finishAbilityPlay(); } };
        const stepRaia = () => { if (cardValue === 'id_33' && window.game.isMyTurn && !me.usedRaia) { window.showConfirm("ライアの効果により、ターン終了時にこのカードを手札に戻しますか？", (yes) => { if (yes) { extraData.returnRaia = true; me.usedRaia = true; } step6(); }); } else { step6(); } };
        const step5 = () => { if (def && def.needsDebuffSelect && me.frozen && me.burnTurns > 0) { window.openDebuffSelection((sel) => { debuffToClear = sel; stepRaia(); }); } else if (def && def.needsDebuffSelect) { if (me.frozen) debuffToClear = 'frozen'; else if (me.burnTurns > 0) debuffToClear = 'burn'; stepRaia(); } else { stepRaia(); } };
        const step4 = () => { if (def && cardValue === 'id_20' && selColor) { const valid = []; window.game.myHand.forEach((c, i) => { if (!indices.includes(i) && i !== discardIdx && c.color === selColor && !(c.value && String(c.value).startsWith('id_'))) valid.push(i); }); if (valid.length > 0) { window.openMultiDiscardSelection(window.game.myHand, valid, (selected) => { multiDiscardIndices = selected; step5(); }); } else { step5(); } } else { step5(); } };
    ...
    // 通常カードのプレイ
    window.game.selectedIndices = []; window.updateUI();
    const indicesToS = [...indices], cardsToS = [...playedCards];
    window.animateSequentialPlay(indices, window.game, () => {
        if (window.isHost) { 
            if (window.socket) window.socket.emit('request_play_animation', { playerId: window.game.myId, cards: cardsToS }); 
            window.executePlay(window.game.myId, indicesToS); 
        } else { 
            window.waitingForServerResponse = true; if (window.socket) window.socket.emit('player_action', { action: 'play', indices: indicesToS, cards: cardsToS }); 
        }
        window.isProcessingPlay = false; 
    });
};
...
window.onColorChosen = function(color) { 
    ColorUI.hide(); if (ColorUI.callback) { let cb = ColorUI.callback; ColorUI.callback = null; cb(color); } else { if (window.isHost) window.executeColor(window.game.myId, color); else { window.waitingForServerResponse = true; if (window.socket) window.socket.emit('player_action', { action: 'color', color: color }); } }
};
...
document.getElementById('draw-btn').onclick = () => {
    ...
    window.tryDrawWithAbility(() => {
        ...
        if (window.SE) window.SE.playMultiple('Distribute', count, 500);
        if (typeof CardAnimation !== 'undefined' && CardAnimation.animateMultiDraw) { CardAnimation.animateMultiDraw(count, 'player-hand', () => { window.isDrawing = false; if (window.isHost) { if (window.socket) window.socket.emit('request_draw_animation', { playerId: window.game.myId, count: count }); window.executeDraw(window.game.myId); } else if (window.socket) window.socket.emit('player_action', { action: 'draw', count: count }); }); } 
        else { window.isDrawing = false; if (window.isHost) { if (window.socket) window.socket.emit('request_draw_animation', { playerId: window.game.myId, count: count }); window.executeDraw(window.game.myId); } else if (window.socket) window.socket.emit('player_action', { action: 'draw', count: count }); }
    });
};
...
document.addEventListener('DOMContentLoaded', () => { if(window.ensureModalsExist) window.ensureModalsExist(); if(window.initVolumeControl) window.initVolumeControl(); });
...
function initMainSocketEvents() {
    ...
    window.socket.on('game_over', (data) => {
        ...
        if (data.isDraw) {
            ...
            if (window.SE) window.SE.play(Math.random() < 0.5 ? 'win' : 'win2'); const { winnerId, winnerName } = data; const target = (winnerId === window.myId) ? document.getElementById('my-player-info') : document.querySelector(`.circle-player-badge[data-id="${winnerId}"]`); if(target) target.classList.add('winner-crown');
    ...
    window.socket.on('receive_player_action', (data) => {
        ...
        if (data.action === 'play') {
            ...
            if (window.game && typeof window.game.replaceAbilityCards === 'function') { window.game.replaceAbilityCards(playerId, data.cards); if (typeof window.broadcastGameState === 'function') window.broadcastGameState(true); }
    ...
    window.socket.on('play_animation', (data) => {
        ...
        if (data.playerId !== window.myId) { if (typeof window.playOpponentAnimation === 'function') window.playOpponentAnimation(data.playerId, data.cards); if (data.cards && data.cards.length > 0 && data.cards[0].value && String(data.cards[0].value).startsWith('id_')) { if (typeof window.showAbilityCutin === 'function') window.showAbilityCutin(data.cards[0].value, data.isHV); } }
    });
    ...
    window.socket.on('draw_animation', (data) => {
        ...
        if (data.playerId !== window.myId) { if (typeof window.drawOpponentAnimation === 'function') window.drawOpponentAnimation(data.playerId, data.count); }
    });
