...
const splashScreen = document.getElementById('splash-screen'), nameInput = document.getElementById('player-name');
const iconInput = document.getElementById('icon-upload'), btnCreateRoom = document.getElementById('btn-create-room');
const btnShowJoin = document.getElementById('btn-show-join'), joinContainer = document.getElementById('join-room-container');
const btnJoinRoom = document.getElementById('btn-join-room'), roomIdInput = document.getElementById('room-id-input');
const loginScreen = document.getElementById('login-screen'), lobbyScreen = document.getElementById('lobby-screen');
const gameContainer = document.getElementById('game-container'), startOverlay = document.getElementById('start-overlay');
const btnStart = document.getElementById('btn-start'), slotList = document.getElementById('slot-list');
...
nameInput.addEventListener('input', (e) => {
    myData.name = e.target.value.trim(); const disabled = myData.name.length === 0;
    ...
    if (file) {
        ...
        reader.onload = (event) => {
            ...
            img.onload = () => {
                ...
                const MAX_SIZE = 100; let width = img.width, height = img.height;
                if (width > height) { if (width > MAX_SIZE) { height *= MAX_SIZE / width; width = MAX_SIZE; } } 
                else { if (height > MAX_SIZE) { width *= MAX_SIZE / height; height = MAX_SIZE; } }
                canvas.width = width; canvas.height = height; ctx.drawImage(img, 0, 0, width, height);
                myData.icon = canvas.toDataURL('image/jpeg', 0.8); document.getElementById('preview-icon').src = myData.icon;
            };
...
btnJoinRoom.addEventListener('click', () => {
    ...
    if (roomId) window.socket.emit('join_room', { roomId: roomId, userData: myData });
});

document.getElementById('btn-disband-room').addEventListener('click', () => {
    if(confirm("ルームを解散しますか？全員が退出します。")) window.socket.emit('disband_room');
});
...
window.socket.on('room_joined', (data) => {
    ...
    if (window.isHost) {
        document.getElementById('btn-add-slot').classList.remove('hidden'); btnStart.classList.remove('hidden'); document.getElementById('btn-disband-room').classList.remove('hidden');
    } else {
        document.getElementById('btn-ready').classList.remove('hidden'); document.getElementById('btn-disband-room').classList.add('hidden');
    ...
    if(data.state && data.state.gameStarted) {
        ...
        if (window.game) { window.game.setup(data.state.slots, data.state.settings); window.game.myId = window.myId; }
    } else {
...
function renderSlots(roomState) {
    ...
    roomState.slots.forEach((slot, index) => {
        ...
        if (slot.type === 'host') {
            ...
            statusSpan.innerText = 'Bot';
        } else {
            statusSpan.innerText = '空き枠';
        }
        ...
        if (window.isHost && index > 0) {
            if (slot.type === 'empty') {
                ...
                botBtn.innerText = '+ Bot';
                botBtn.className = 'add-bot-btn';
                botBtn.onclick = () => window.socket.emit('toggle_bot', index);
                div.appendChild(botBtn);
            } else if (slot.type === 'bot') {
                const botBtn = document.createElement('button');
                botBtn.innerText = '- Bot';
                botBtn.className = 'remove-bot-btn';
                botBtn.onclick = () => window.socket.emit('toggle_bot', index);
                div.appendChild(botBtn);
            }
