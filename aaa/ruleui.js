...
window.CUSTOM_CARDS_DEF = [
    ...
    { id: 'id_20', name: '幽艶レベッカ', desc: '【HV】トリック・オア・キャロット。色を指定でき、指定した色以外のカードを全て山札に戻す。(記号効果なし/既存カードのみ適応)' },
    ...
    { id: 'id_25', name: 'ミサ', desc: '【HV】自カード1枚をワイルドにし、使用されたSSR以下の能力カードを1枚回収。使用後、蘇生・ミサが0なら付与(強制ドロー時に手札に戻る)。' },
    { id: 'id_26', name: '運命の三姉妹', desc: '【HV】ランダム他1人とじゃんけん。初回は結果によらず相手2ドロー。勝つかあいこで相手2ドローし自分は通常カード1枚破棄。その後別の人と再戦(最大4回)。' },
    ...
    { id: 'id_33', name: 'ライア', desc: '【AT】自分以外のプレイヤーを一人指定し1枚ドローさせる。発動後このカードを手札に戻してもよい。(各ターン1回のみ)' },
    { id: 'id_34', name: 'オリヴィア', desc: '【HE】自分に回避I(20%の確率で攻撃を防ぐ)を1ターン付与する。' },
    { id: 'id_35', name: 'イヴ', desc: '【HV】ランダム1名に燃焼2T、他全員に裂傷(ドロー時+1枚)2T付与。使用後、蘇生・イヴが0なら付与(被強制ドロー時に手札に戻る)。' }
];
...
if (typeof window !== 'undefined' && !window.RuleUIManager) {
    window.RuleUIManager = {
        ...
        broadcastSettings: function() {
            ...
            const settings = {
                initialHandSize: document.getElementById('setting-initial-hand').value,
                initialCustomHandSize: this.initialCustomHandSize,
                customCards: this.customCards,
                unoPenalty: document.getElementById('setting-uno-penalty').value,
                unoAuto: document.getElementById('setting-uno-auto').checked,
                allowActionFinish: document.getElementById('setting-allow-action-finish').checked,
                actionFinishPenalty: document.getElementById('setting-action-finish-penalty').value,
                allowAbilityFinish: document.getElementById('setting-allow-ability-finish').checked,
                abilityFinishPenalty: document.getElementById('setting-ability-finish-penalty').value,
                maxMultiPlay: document.getElementById('setting-max-multi-play').value,
                maxDrawMultiPlay: document.getElementById('setting-max-draw-multi-play').value,
                randomTurnOrder: document.getElementById('setting-random-turn-order').checked,
                optionalDraw: document.getElementById('setting-optional-draw').checked,
                abilityResetCount: document.getElementById('setting-ability-reset-count').value,
                showBotPersonality: document.getElementById('setting-bot-personality').checked
            };
