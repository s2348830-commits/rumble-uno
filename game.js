...
class UNOGame {
    ...
    setup(slots, ruleSettings) {
        ...
        colors.forEach(c => {
            values.forEach(v => {
                pool.push({ color: c, value: v });
                if (v !== '0') pool.push({ color: c, value: v });
            });
        });
        for(let i=0; i<4; i++) {
            pool.push({ color: 'black', value: 'Wild' });
            pool.push({ color: 'black', value: 'Wild+4' });
        }
    ...
    start() {
        ...
        this.players.forEach(p => {
            ...
            for(let i=0; i<initialCustomSize; i++) {
    ...
    playCards(playerId, indices) {
        try {
            ...
            // ★修正: 出した後の手札枚数で正しく上がりペナルティを判定する
            let remainingCards = hand.length - indices.length;
            if (isAbility && def && (def.needsDiscard || def.needsAbilityDiscard)) remainingCards -= 1;

            if (remainingCards <= 0) {
                if (isAbility && this.ruleSettings && !this.ruleSettings.allowAbilityFinish) {
                    return { success: false, penalty: true, penaltyReason: '能力' };
                }
                if (!isAbility && !/^[0-9]$/.test(cards[cards.length - 1].value) && this.ruleSettings && !this.ruleSettings.allowActionFinish) {
                    return { success: false, penalty: true, penaltyReason: '記号' };
                }
            }

            if (isAbility) {
                return { success: true, isAbility: true };
            }
            ...
            if (lastCard.value === 'Skip') this.nextTurn(1 + cards.length);
                ...
                if (this.players.length === 2 && cards.length % 2 !== 0) this.nextTurn(2);
                else this.nextTurn(1);
            }
            else if (lastCard.value === '+2') {
                this.drawStack += 2 * cards.length;
                this.nextTurn(1);
            }
            else if (lastCard.value === 'Wild') {
                needsColor = true;
            }
            else if (lastCard.value === 'Wild+4') {
                this.drawStack += 4 * cards.length;
                needsColor = true;
            }
            else {
                this.nextTurn(1);
    ...
    nextTurn(steps = 1) {
        for (let i = 0; i < steps; i++) {
            ...
            } while (!this.players[this.turnIndex].connected);
        }
        this.hasDrawnThisTurn = false;
        const currentP = this.players[this.turnIndex];
        if (currentP.burnTurns > 0) {
            this.drawCard(currentP.id);
            currentP.burnTurns--;
        }
        if (currentP.shield && currentP.shield.turns > 0) {
            currentP.shield.turns--;
            if (currentP.shield.turns <= 0) currentP.shield.level = 0;
        }
        if (currentP.evasion && currentP.evasion.turns > 0) {
            currentP.evasion.turns--;
            if (currentP.evasion.turns <= 0) currentP.evasion.level = 0;
        }
        currentP.usedRaia = false; 

        if (currentP.frozen) {
            let hasAbility = false;
            if (this.hands[currentP.id]) {
                hasAbility = this.hands[currentP.id].some(c => c && c.value && String(c.value).startsWith('id_'));
            }
            if (!hasAbility) {
                currentP.frozen = false;
                setTimeout(() => this.nextTurn(1), 1000); 
            }
        }
        
        if (this.hands[currentP.id]) {
            this.hands[currentP.id].forEach(c => {
                if (c && c.lockedTurns > 0) c.lockedTurns--;
            });
        }
    }
}