...
window.AbilityDef = {
    ...
    'id_20': { rarity: 'UR', type: 'HV', name: '幽艶レベッカ', desc: '【HV】トリック・オア・キャロット。色を指定でき、指定した色以外のカードを全て山札に戻す。(記号効果なし/既存カードのみ適応)', needsColor: true },
    ...
    'id_25': { rarity: 'UR', type: 'HV', name: 'ミサ', desc: '【HV】自カード1枚をワイルドにし、使用されたSSR以下の能力カードを1枚回収。使用後、蘇生・ミサが0なら付与(強制ドロー時に手札に戻る)。', needsGraveyard: true },
        ...
        for (let m = 0; m < multiplier; m++) {
                ...
                actualTargets.forEach(t => {
                    ...
                    if (abilityId === 'id_1') {
                        ...
                        if (Math.random() < 0.75) {
                            ...
                            guides.push({ from: attackerId, to: targetId, text: 'ハズレ!' });
                        }
            ...
            if (def.type === 'HE' || def.type === 'HE_BL' || def.type === 'HV') {
                if (abilityId === 'id_3' && costPaid) {
                    ...
                    if (res === -1) guides.push({ from: attackerId, to: selectedTargetId, text: '💨回避!' });
                    else guides.push({ from: attackerId, to: selectedTargetId, text: '1枚引かせる' });
